...
control MyIngress(inout headers hdr,
    ...
    action set_ecmp_select(bit<16> ecmp_base, bit<32> ecmp_count) {
        ...
        hash(
            ...
            ecmp_count             // 哈希的范围
        ); // 所得值为[ecmp_base, ecmp_base + ecmp_count - 1]，控制平面下发信息可得为：[0, 1]
    }
    ...
    table ecmp_group {
        ...
        actions = {
            drop;
            set_ecmp_select; // 当目标IP为10.0.0.1时，选择ecmp转发
        }
        size = 1024;
    }
    table ecmp_nhop {
    ...
    apply {
        ...
        if(hdr.ipv4.isValid() && hdr.ipv4.ttl > 0) {
            // 如果ipv4字段有效，且ipv4的ttl不为0，选择进行负载均衡
            ecmp_group.apply();
            ecmp_nhop.apply();
        }
...
control MyEgress(inout headers hdr,
    ...
    table send_frame {
        key = {
            standard_metadata.egress_port: exact; // 匹配出端口
        }
        actions = {
            rewrite_mac;
            drop;
